...
# Get the auto-created node security group
...
# Add security group rule to allow ALB to reach pods
resource "aws_security_group_rule" "node_group_alb_ingress" {
  type              = "ingress"
  from_port         = 8080
  to_port           = 8080
  protocol          = "tcp"
...
# Add security group rule to allow ALB to reach NodePort services
resource "aws_security_group_rule" "node_group_nodeport_ingress" {
  type              = "ingress"
  from_port         = 30000
  to_port           = 32767
  protocol          = "tcp"
...
# EKS Access Entry for Christian's IAM role
resource "aws_eks_access_entry" "christian_role" {
  cluster_name  = aws_eks_cluster.main.name
  principal_arn = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/christian"
  type          = "STANDARD"
}

resource "aws_eks_access_policy_association" "christian_role_admin" {
  cluster_name  = aws_eks_cluster.main.name
  principal_arn = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/christian"
  policy_arn    = "arn:aws:eks::aws:cluster-access-policy/AmazonEKSClusterAdminPolicy"
  ...
  depends_on = [aws_eks_access_entry.christian_role]
}

# EKS Access Entry for Christian's IAM user
resource "aws_eks_access_entry" "christian_user" {
  cluster_name  = aws_eks_cluster.main.name
  principal_arn = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:user/cmoore"
  type          = "STANDARD"
}

resource "aws_eks_access_policy_association" "christian_user_admin" {
  cluster_name  = aws_eks_cluster.main.name
  principal_arn = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:user/cmoore"
  policy_arn    = "arn:aws:eks::aws:cluster-access-policy/AmazonEKSClusterAdminPolicy"
  ...
  depends_on = [aws_eks_access_entry.christian_user]
}
